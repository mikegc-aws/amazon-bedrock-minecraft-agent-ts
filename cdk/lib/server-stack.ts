...
export class ServerStack extends cdk.Stack {
  ...
  const container = taskDefinition.addContainer('MinecraftServerContainer', {
    ...
    logging: ecs.LogDriver.awsLogs({ streamPrefix: 'minecraft-server' }),
    portMappings: [{ 
  ...
  const agentContainer = taskDefinition.addContainer('NodeAgentContainer', {
    ...
    logging: ecs.LogDriver.awsLogs({ streamPrefix: 'node-agent' }),
    ...
    healthCheck: {
      ...
      retries: 3,
      startPeriod: Duration.minutes(1), // 1 minutes
    },
    portMappings: [{
      containerPort: 3000,
      hostPort: 3000,
